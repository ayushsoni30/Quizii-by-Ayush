...
export const fetchQuestions = async (amount = 10, categoryId = null, difficulty = null, retries = 3) => {
  try {
    ...
    if (error.response?.status === 429 && retries > 0) {
      console.warn('Rate limited (429) fetching questions. Retrying in 3 seconds...');
      await delay(3000);
      return fetchQuestions(amount, categoryId, difficulty, retries - 1);
    }
    console.error('Error fetching questions:', error);
    throw new Error(error.response?.status === 429 ? "Too many requests to the Trivia API. Please wait a moment and try again." : error.message);
  }
};
